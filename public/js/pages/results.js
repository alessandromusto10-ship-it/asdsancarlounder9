...
document.getElementById('match-info').innerHTML = `<div style="font-size: 13px; color: var(--gray-500); margin-bottom: 4px;"> ${homeName} vs ${awayName} </div>
`;
...
if (homeScore + awayScore > 3) {
 ...
 try {
   ...
   // ✅ INVIA NOTIFICA RISULTATO A TUTTI
   await this.sendResultNotification(this.currentMatchId, homeScore, awayScore);
   
 } catch (err) {
   toast('Errore: ' + err.message, 'error');
 }
},
// ✅ NUOVA FUNZIONE: Invia notifica a tutti quando viene aggiornato un risultato
async sendResultNotification(matchId, homeScore, awayScore) {
  try {
    console.log('📤 Invio notifica risultato...');
    
    // Recupera i dettagli della partita
    const { data: matchData, error: matchError } = await db
      .from('matches')
      .select(`
        id, match_date, match_time, location,
        home_team:teams!matches_home_team_id_fkey(name),
        away_team:teams!matches_away_team_id_fkey(name)
      `)
      .eq('id', matchId)
    ...
    if (matchError) {
      console.error('Errore recupero partita:', matchError);
      return;
    }
    ...
    if (subsError) {
      console.error('Errore recupero subscription:', subsError);
      return;
    }
    
    // Estrai user_id unici
    const userIds = [...new Set(subscriptions.map(s => s.user_id))];
    
    if (userIds.length === 0) {
      console.warn('Nessuna subscription trovata');
      return;
    }
    ...
    if (success) {
      console.log('✅ Notifica risultato inviata a', userIds.length, 'utenti');
    }
  } catch (err) {
    console.error('❌ Errore invio notifica risultato:', err);
  }
}
};
